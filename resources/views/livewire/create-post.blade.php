<div x-cloak>
    <form wire:submit.prevent="post" method="POST" enctype="multipart/form-data" class="z-40 playfair-regular" id="blogForm">
        ...
        <div class="mt-5">
            @if ($image)
                <div wire:loading class="animate-pulse flex items-center justify-center w-full h-48 md:h-90 mb-7 p-2 border-dashed border-7 rounded-md border-stone-300" id="imagePartial">
            ...
            @else
                <label for="images" class=" @error('image') animate-shake @enderror flex items-center justify-center w-full h-48 md:h-90 mb-7 p-2 border-dashed border-7 rounded-md border-stone-300">
                    <div class="bg-stone-300 rounded-md w-full h-full flex items-center justify-center">
